  ...
  <transition name="drawer">
    <div v-if="isOpen" class="fixed inset-0 z-[200] overflow-hidden">
      ...
      <div class="absolute inset-y-0 right-0 max-w-full flex">
        <div class="relative w-screen max-w-md bg-white shadow-2xl flex flex-col">
          <!-- Header -->
          <div class="px-6 py-6 border-b border-gray-100 flex items-center justify-between">
            ...
            <button @click="close" class="p-2 hover:bg-gray-100 rounded-xl transition-all">
              <Icon name="lucide:x" size="24" class="text-gray-400" />
            </button>
          </div>
          ...
          <div class="flex-1 overflow-y-auto px-6 py-8 no-scrollbar">
                ...
                <div class="flex-1 flex flex-col justify-between py-1">
                  ...
                  <div class="flex items-center justify-between pt-2">
                    <div class="flex items-center bg-gray-50 rounded-xl border border-gray-100 p-1">
                      <button @click="updateQuantity(item.productId, item.quantity - 1)" class="w-7 h-7 flex items-center justify-center hover:bg-white rounded-lg transition-all text-gray-400 hover:text-gray-950">
                    ...
                    <button @click="removeItem(item.productId)" class="text-[10px] font-black uppercase tracking-widest text-red-400 hover:text-red-500 transition-colors">
                      Remove
                    </button>
                  </div>
                </div>
            ...
            <div v-else class="h-full flex flex-col items-center justify-center text-center space-y-6">
              ...
              <div class="space-y-2">
                ...
                <p class="text-gray-400 text-sm font-medium">Looks like you haven't added anything yet.</p>
              </div>
              <button @click="close" class="bg-[#033958] text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-gray-950 transition-all shadow-xl shadow-[#033958]/20">Start Shopping</button>
            </div>
          </div>
          ...
          <div v-if="items.length > 0" class="px-6 py-8 border-t border-gray-100 bg-gray-50/50 space-y-6">
            <div class="flex items-center justify-between">
              <span class="text-sm font-black text-gray-400 uppercase tracking-widest">Subtotal</span>
              <span class="text-2xl font-black text-gray-950 tracking-tight">{{ formatPrice(totalPrice) }}</span>
            </div>
            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest text-center italic">Taxes and shipping calculated at checkout</p>
            <NuxtLink to="/checkout" @click="close" class="w-full bg-[#033958] text-white py-6 rounded-[2rem] font-black text-sm uppercase tracking-widest hover:bg-gray-950 transition-all shadow-2xl shadow-[#033958]/20 flex items-center justify-center gap-4 group">
              Checkout Now
              <Icon name="lucide:arrow-right" size="20" class="group-hover:translate-x-2 transition-transform" />
            </NuxtLink>
            <button @click="close" class="w-full text-xs font-black text-gray-400 uppercase tracking-widest hover:text-gray-950 transition-colors">Continue Shopping</button>
          </div>
        </div>
      </div>
    </div>
