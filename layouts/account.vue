  ...
  <div class="min-h-screen bg-gray-50 flex flex-col">
    ...
    <header class="bg-white border-b border-gray-100 z-30">
      <div class="max-w-[1440px] mx-auto px-4 h-16 flex items-center justify-between">
        <NuxtLink to="/" class="flex items-center gap-2 group">
          <Icon name="lucide:arrow-left" class="w-4 h-4 text-[#033958]/80 group-hover:text-gray-900 transition-colors" />
          <span class="text-xs font-black uppercase tracking-widest text-[#033958]/80 group-hover:text-gray-900 transition-colors mt-0.5">Back to Shop</span>
        </NuxtLink>
        <div class="flex items-center gap-3">
          <div class="text-right">
            <p class="text-xs font-black uppercase tracking-widest text-[#033958] leading-none">{{ user?.role || 'Customer' }}</p>
            <p class="text-xs font-bold text-gray-900">{{ user?.fullName }}</p>
          </div>
          <div class="w-8 h-8 rounded-full bg-[#033958] flex items-center justify-center text-white text-xs font-black">
    ...
    <div class="flex-1 flex max-w-[1440px] mx-auto w-full relative">
      ...
      <aside class="hidden lg:block w-72 bg-white border-r border-gray-100 py-10 px-6 shrink-0 h-[calc(100vh-64px)] sticky top-16">
        <div class="space-y-8">
          <div>
            <h3 class="text-xs font-black uppercase tracking-[0.2em] text-[#033958]/80 mb-6 px-4">Workspace</h3>
            <nav class="space-y-1">
              <NuxtLink v-for="item in menu" :key="item.path" :to="item.path" 
                ...
                :class="(item.path === '/account' ? route.path === '/account' : route.path.startsWith(item.path)) ? 'bg-[#033958] text-white  /20' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'"
              >
                <Icon :name="item.icon" class="w-5 h-5" :class="(item.path === '/account' ? route.path === '/account' : route.path.startsWith(item.path)) ? 'text-amber-400' : 'text-[#033958]/80 group-hover:text-gray-900'" />
      ...
      <main class="flex-1 p-4 lg:p-10 min-h-0 min-w-0 bg-gray-50/50">
        <div class="bg-white rounded-[32px] border border-gray-100  min-h-full p-6 lg:p-10 relative overflow-hidden">
          <slot />
        </div>
      </main>
    </div>
